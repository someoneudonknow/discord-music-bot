...
const EmbedGenerator_1 = __importDefault(require("./EmbedGenerator"));
exports.default = async (tracks, limit, userId, replyFunc) => {
    ...
    for (let page = 0; page < maxPage; page++) {
        ...
        if (!slicedTracks)
            continue;
        const embed = EmbedGenerator_1.default.Info({
            description: slicedTracks
                .map((track, index) => `**${start + index}**. [${track.title}](${track.url}) ~ [Requested by ${track?.requestedBy?.toString()}]`)
                .join("\n"),
            footer: {
                text: `Page ${page + 1} of ${maxPage} | Showing songs ${start + 1} to ${end} of ${slicedTracks.length}`,
            },
        });
        embeds.push(embed);
    }
    const row = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
        ...
        .setEmoji("▶️")
        .setStyle(discord_js_1.ButtonStyle.Secondary)
        .setDisabled(maxPage <= 1), new discord_js_1.ButtonBuilder()
        .setCustomId("lastBtn")
        .setEmoji("⏩")
        .setStyle(discord_js_1.ButtonStyle.Secondary)
